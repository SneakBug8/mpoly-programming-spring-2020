...
public class BrickPile {
	...
	public BrickPile(PlayField pf, Image img) {
		...
		for (int r = 0; r < _rows; r++) {
			for (int c = 0; c < _cols; c++) {
				...
				/*
				 * _lib[2] = _pf.getToolkit().getImage("src/Briks/brick.gif"); _lib[3] =
				 * _pf.getToolkit().getImage("src/Briks/wallbrick.gif"); _lib[4] =
				 * _pf.getToolkit().getImage("src/Briks/superbrick.gif");
				 */
				Brick b;
				if (r == 0) {
				...
				} else if (r == _rows - 1 && c % 2 == 0) {
					b = new BonusBrick(_pf, this, pos, Match._lib[7]);
				} else if (r == _rows -1) {
		...
		//var fallingball = new FallingBall(_pf, Match._ps, Match._lib[6], new Vector2(100, 100));
		//Match._pf.addSprite(fallingball);
		//fallingball._isDead = true;
	}
	...
	public int unbrokenCount() {
		...
		for (int i = 0; i < _bricks.size(); i++) {
			if (!((Brick) _bricks.elementAt(i)).isDead() && _bricks.elementAt(i).getClass() != WallBrick.class)
				result++;
		}
